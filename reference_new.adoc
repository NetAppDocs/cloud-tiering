...
Inactive data reporting is now automatically enabled on ONTAP clusters that you discover through Cloud Manager. This enhancement makes it easier for Cloud Tiering to show you the potential savings for tiering cold data.

=== Remove clusters

You can now remove failed clusters from the dashboard.

image:screenshot_remove_cluster.gif[] (screenshot coming soon)
