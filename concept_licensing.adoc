...
After you purchase a FabricPool license, you'll need to add it to the cluster, link:task_licensing.html[which you can do directly from Cloud Tiering].

If you activate your license through Cloud Tiering and then you purchase additional add-on capacity at a later time, the license is automatically updated on the cluster with the new capacity. There's no need to apply a new NetApp License File (NLF) to the cluster.
