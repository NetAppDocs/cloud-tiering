...
== Changing the tiering policy for a volume
...
. From the *Cluster Dashboard*, click *Set up Tiering* for the cluster.

. *Source Volumes*: Choose a different tiering policy for one or more volumes.
+
image:screenshot_change_tiering_policy.gif[A screenshot of the Source Volumes screen where two volumes are selected and "Cold user data" is the tiering policy.]

. *Review*: Review your selections and click *Go*.
+
image:screenshot_change_tiering_policy_review.gif[A screenshot of the Review & Approve page which shows a summary of changes to two volumes.]
